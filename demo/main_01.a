...
# 3. 聊天历史管理，按 session_id 存储多轮对话，临时 memory 会话结束自动销毁
chats_by_session_id = {}
...
chain = (
    RunnablePassthrough.assign(
        chat_history=lambda x: get_chat_history(x["session_id"]).messages
    )
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        pass
    # 首次进入，发送欢迎语（input 传空字符串，严格按 prompt 规则）
    ...
    try:
        while True:
            ...
            if user_input.lower() in ["exit", "quit", "q"]:
                ...
                remove_chat_history(session_id)  # 断开时自动销毁 memory
                return
            result = chain.invoke({"input": user_input, "session_id": session_id})
            await websocket.send_text(result)
            await websocket.send_text("[END]")
    except WebSocketDisconnect:
        remove_chat_history(session_id)  # 客户端断开也自动销毁 memory
        pass