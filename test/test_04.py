...
try:
    print("\n[DEBUG] 首次触发欢迎语...")
    # 模拟一个空的用户消息，以便 LLM 能够根据提示词规则生成欢迎语
    # 注意：这里我们不向 current_chat_history["messages"] 添加这个空消息，
    # 仅仅是作为第一次 invoke 的触发条件。
    # LLM 会根据 SYSTEM_PROMPT_TEMPLATE 中的“首次进入时”规则来判断。
    
    # 为了让 LLM 知道这是“首次进入”，可以传入一个非常规的初始 HumanMessage，
    # 或者直接让 LLM 依赖 SYSTEM_PROMPT 的强大理解能力。
    # 在这个场景下，传入一个空的 HumanMessage 是触发其遵循“首次进入”规则的有效方式。
    
    # 构建一个临时的消息列表，包含一个空的HumanMessage来触发LLM
    # 这样可以确保LLM接收到至少一条用户侧的消息（即使内容为空）来启动其逻辑
    temp_initial_messages = [HumanMessage(content="")] 
    
    # 复制当前状态，并传入临时消息，避免修改原始 current_chat_history
    # 这样做的目的是确保 app.invoke 接收到一个包含“用户输入”的 state，
    # 从而让 route_decision 和 llm_node 可以正常处理。
    # 并且，由于这个 HumanMessage 内容为空，它不会影响后续的对话逻辑判断，
    # 却能有效地触发 LLM 生成欢迎语。
    
    initial_invoke_state = {"messages": temp_initial_messages, "word": initial_word}
    
    # 第一次调用图，只为了获取欢迎语
    result_state_first_invoke = app.invoke(initial_invoke_state)
    
    # 将第一次 invoke 的结果更新到主历史中
    current_chat_history = result_state_first_invoke
    
except Exception as e:
    print(f"\n首次欢迎语生成发生错误: {e}")
    import traceback
    traceback.print_exc()
    # 如果首次生成失败，我们应该选择退出或继续，这里选择继续，让用户手动输入
    pass # 忽略错误，继续主循环
