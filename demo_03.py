...
def call_model(state: MessagesState, config: RunnableConfig) -> dict:
    ...
    # 构造消息序列：system + 历史 + 当前
    messages = [HumanMessage(content=SYSTEM_PROMPT_TEMPLATE.format(word="boy"))] + list(chat_history.messages) + state["messages"]
...
if __name__ == "__main__":
    ...
    word = "apple"  # 可替换为用户输入的单词
    system_prompt = SYSTEM_PROMPT_TEMPLATE.format(word=word)
    print("助手：", end="", flush=True)
    # 首次进入，传入单词和欢迎语
    input_message = HumanMessage(content=word)
    for event in graph.stream({"messages": [HumanMessage(content=system_prompt), input_message]}, config, stream_mode="values"):
